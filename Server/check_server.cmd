...
tasklist /FI "imagename eq ddctd_cm_yo_serve.exe" /V | find "world ID %4" >NUL && goto :WAITCLOSE
echo done.
ping ::1 -n 5 >NUL 2>NUL

:LOOP
REM If server is started, exit batch file.
tasklist /FI "imagename eq ddctd_cm_yo_serve.exe" /V | find "world ID %4" >NUL && goto :END

REM If not, try to start it.
echo Launching WorldID %4.
start ddctd_cm_yo_serve.exe -WorldID %4
